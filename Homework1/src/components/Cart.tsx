...
interface CartProps {
  ...
  removeFromCart: (id: number) => void;  // เพิ่มส่วนนี้
  totalPrice: number;
}
      ...
      <div className="container mx-auto px-4 py-8 border rounded-lg bg-white shadow-md hover:shadow-xl transition-shadow w-5/6">
        ...
        <div className="flex flex-wrap justify-center items-start">
          <div className="mt-4">
            ...
            {cartItems.length === 0 ?  
            EmptyCart() : BooksInCart(cartItems, updateQuantity, removeFromCart ,totalPrice )}
...
function BooksInCart(
  cartItems: CartItem[], 
  updateQuantity: (bookId: number, change: number) => void,
  removeFromCart: (id: number) => void,  // เพิ่ม parameter นี้
  totalPrice: number
) {
  return <>
    {cartItems.map((item) => (
      <CartDetail
        ...
        removeFromCart={removeFromCart}  // เพิ่ม prop นี้
      />
    ))}
  </>;
}
